...
class CDatabaseController {
	...
	public function MultiQuery($aQuery) {
		$res = $this->iMysqli->multi_query ( $aQuery ) or die ( "Could not query database, query =<br/><pre>{$aQuery}</pre><br/>{$this->iMysqli->error}" );
	...
	public function MultiQuerySpecial($aQuery) {
		...
		if ($res == null) {
			if ($this->iMysqli->errno == 1062) {
				...
				die ( "Could not query database, query =<br/><pre>{$aQuery}</pre><br/>{$this->iMysqli->error}" );
		}
	...
	public function RetrieveAndStoreResultsFromMultiQuery(&$aResults) {
		...
		! $mysqli->errno or die ( "<p>Failed retrieving resultsets.</p><p>Query =<br/><pre>{$query}</pre><br/>Error code: {$this->iMysqli->errno} ({$this->iMysqli->error})</p>" );
	}
	...
	public function Query($aQuery) {
		$res = $this->iMysqli->query ( $aQuery ) or die ( "Could not query database, query =<br/><pre>{$aQuery}</pre><br/>{$this->iMysqli->error}" );
